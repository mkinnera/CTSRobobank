...
public class CustomerStatementControllerTest {
    @InjectMocks
    CustomerFileValidationController customerFileValidationController;
    @Mock
    ...
    public void processTest() throws IOException, RecordParseException, ResourceNotFoundException {
        ...
        is.close();
        when(customerFileValidationService.process(any(), any())).thenReturn(records);
        ResponseEntity<Response> list = customerFileValidationController.validateFile(multipartFile);
    ...
    public void processTestWithResourceNotFoundException() throws IOException, RecordParseException, ResourceNotFoundException {
        ...
        MockMultipartFile multipartFile = new MockMultipartFile("csv", "records.csv", "text/csv", is);
        is.close();
        when(customerFileValidationService.process(any(), any())).thenThrow(new ResourceNotFoundException("should not be empty"));
        customerFileValidationController.validateFile(multipartFile);
    }
